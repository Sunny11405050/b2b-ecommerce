...
{	
	...
	public function __construct($id,$module=null){
		...
		try {
			...
			// Ensure the language is valid and not too long
			if(!empty(Yii::app()->language) && strlen(Yii::app()->language) >= 10){
				Yii::app()->language = KMRS_DEFAULT_LANGUAGE;
			}
		} catch (Exception $e) {
			Yii::log("Error setting language: " . $e->getMessage(), CLogger::LEVEL_ERROR);
			Yii::app()->language = KMRS_DEFAULT_LANGUAGE;
		}
	}
	...
	{	
		try {
			...
			Yii::log("Error initializing settings: " . $e->getMessage(), CLogger::LEVEL_ERROR);
		}			
	}
